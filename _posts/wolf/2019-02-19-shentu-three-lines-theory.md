...
先分析一下，`4` 个狼人随机分发到位置 `1` 到 `12` 中的所有可能总数，根据排列组合的知识，这个值是 $$C_{12}^4$$ ，即总的可能情况数为：

$$C_{12}^4 = (12 * 11 * 10 * 9) / (4 * 3 * 2 * 1) = 11880 / 24 = 495$$ 种可能。
...
$$C_6^2 * 1 * 1 = (6 * 5) / (2 * 1) * 1 * 1 = 15 * 1 = 15$$ 种可能。
...
我们先考虑 “`1-2-1 类型`” 中的 `2`，即 `C` 这一行，跟前面 “`2-2`” 类型类似，这种情况没有选择，有且一种可能性。但是从上往下看，这个 `2` 可能是这三行中靠最前的，或是中间的，又或是靠最后的，例如上图中，这个 `2` 就是靠中间的，也就是 `1-2-1` 这种类型。在前面我们也分析过了，分布的格式一共有 `2-1-1`, `1-2-1`, `1-1-2` 三种不同可能，所以这个 `2` 的可能共有 $$3 * 1 = 3$$ 种可能；
...
$$C_6^3 * 3 * 2 * 2 = (6 * 5 * 4) / (3 * 2 * 1) * 3 * 2 * 2 = 20 * 12 = 240$$ 种可能。
...
$$C_6^4 * 2 * 2 * 2 * 2 = (6 * 5) / (2 * 1) * 2 * 2 * 2 * 2 = 15 * 16 = 240$$ 种可能。

所以“`1-1-1-1 类型`”出现的概率是：$$P = 15 / 495 = 48.4848 \% $$。

### 2.3.4. 简单验证

把以上三种类型的可能情况的总数合计一下，可以得到：
...
这里可以给个提示，就是，如果要找狼人，说明你肯定是一个好人，因为在标准局里，狼人是互相认识的。所以上面的分析是有一点小瑕疵的，但要得到正确的结果，分析过程是比上面的过程要更复杂的，敬请期待。
